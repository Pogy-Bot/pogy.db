...
module.exports = {
   ...
   */
  connect: async function (url, options, databaseOptions) {
   ...
   */
  migrate: async function (schema, newConnection, options) {
    ...
    try {
      ...
      try {
        ...
        step = 6;
        const newTable = await newConnectionDatabase.model(schema, {
          id: String,
          data: Object,
        });
  ...
  table: function (tableName) {
    return (async () => {
      ...
      this.subtract = async function (key, value) {
        if (!key)
          throw new TypeError(
            "No key specified. Need Help ? Visit pogy.xyz/support"
          );
